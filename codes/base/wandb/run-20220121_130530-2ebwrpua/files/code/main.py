...
def _train(cfg, _run, ex, tensorboard):
    ...
    for task_i in range(inc_dataset.n_tasks):
        task_info, train_loader, val_loader, test_loader, fake_train_loader = inc_dataset.new_task()
        model.test_loader = test_loader
        model.train_loader = train_loader
        model.fake_train_loader = fake_train_loader
        ...
        models_name = ['incmodel','model_imagenet','ablation','generative']
        if cfg["model"] in models_name:
            acc_stats = utils.compute_accuracy_ema(ypred, ytrue, increments=model._increments, n_classes=model._n_classes)
        else:
        ...
        if cfg["model"] in models_name:
        # if isinstance(acc_stats, list) or isinstance(acc_stats, tuple):
            _run.log_scalar(f"trial{trial_i}_taskaccu", acc_stats[0]["top1"]["total"], task_i)
            _run.log_scalar(f"trial{trial_i}_task_top5_accu", acc_stats[0]["top5"]["total"], task_i)
            _run.log_scalar(f"trial{trial_i}_taskaccu_ema", acc_stats[1]["top1"]["total"], task_i)
            _run.log_scalar(f"trial{trial_i}_task_top5_accu_ema", acc_stats[1]["top5"]["total"], task_i)

            ex.logger.info(f"top1:{acc_stats[0]['top1']}")
            ex.logger.info(f"top1 ema:{acc_stats[1]['top1']}")
            ex.logger.info(f"top5:{acc_stats[0]['top5']}")
            ex.logger.info(f"top5 ema:{acc_stats[1]['top5']}")

            results["results"].append(acc_stats[0])
            results["results ema"].append(acc_stats[1])
        else:
    ...
    # if len(results["results ema"]) == 0:
    if cfg["model"] == "baseline" or cfg["model"] == "regularization":
        ...
        top1_avg_acc_ema, top5_avg_acc_ema = results_utils.compute_avg_inc_acc(results["results ema"])
        _run.info[f"trial{trial_i}"][f"avg_incremental_accu_top1_ema"] = top1_avg_acc_ema
        _run.info[f"trial{trial_i}"][f"avg_incremental_accu_top5_ema"] = top5_avg_acc_ema
        ex.logger.info("Average Incremental Accuracy Top 1: {} {} Top 5: {} {}.".format(
                _run.info[f"trial{trial_i}"][f"avg_incremental_accu_top1"],
                _run.info[f"trial{trial_i}"][f"avg_incremental_accu_top1_ema"],
                _run.info[f"trial{trial_i}"][f"avg_incremental_accu_top5"],
                _run.info[f"trial{trial_i}"][f"avg_incremental_accu_top5_ema"],
            ))
...
def test(_run, _rnd, _seed):
    ...
    cfg.data_folder = '/home/share/fengjw/CIFAR100'
    inc_dataset = factory.get_data(cfg, trial_i)
