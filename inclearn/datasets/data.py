...
class IncrementalDataset:
    def __init__(
        ...
        resampling=False,
        data_folder="/home/share/jiawei/DER-ClassIL.pytorch/data/",
        start_class=0,
    ):
        ...
        #Dataset Info
        #-------------------------------------
        self.data_folder =data_folder# get_data_folder(data_folder, dataset_name)
    ...
    def new_task(self):
        ...
        fake_x_train, fake_y_train = self._get_cur_step_data_for_fake_data()
        ...
        fake_train_loader = self._get_fake_loader(fake_x_train, fake_y_train)
        ...
        task_info = {
            ...
            "n_test_data": len(y_train), #TODO wrong??
        }

        self._current_task += 1
        return task_info, train_loader, val_loader, test_loader, fake_train_loader
    ...
    def _get_cur_step_data_for_fake_data(self, ):
        min_class = sum(self.increments[:self._current_task])
        max_class = sum(self.increments[:self._current_task + 1])

        x_train, y_train = self._select(self.fake_data_train, self.fake_targets_train, low_range=min_class, high_range=max_class)
    ...
    def _setup_fake_data(self):
        self.fake_data_train = []
        self.fake_targets_train = []
        xhat_rootpath = '/home/fengjw/HAN-master/experiment/HANx2_test/results-cifar100/'
        img_paths = glob.glob(os.path.join(xhat_rootpath, '*.png'))
        ...
        for img_path in img_paths:
            idx = int(pattern.search(img_path).groups()[0])
            if not cnt == idx:
                import pdb
                pdb.set_trace()

            label = int(pattern.search(img_path).groups()[1])
            xhat = Image.open(img_path).convert('RGB')
            self.fake_data_train.append(np.array(xhat))
            self.fake_targets_train.append(label)
            cnt += 1
        self.fake_targets_train = np.array(self.fake_targets_train)
        self.fake_data_train = np.array(self.fake_data_train)
        print(self.fake_data_train.shape,self.fake_targets_train.shape)
    ...
    def _setup_data(self, dataset):
        ...
        print(self.data_train.shape,self.targets_train.shape)
    ...
    def _setup_data_for_raw_data(self, dataset, train_dataset, test_dataset, current_class_idx=0):
        ...
        if "imagenet" in self.dataset_name:
            x_train = x_train[0:129395]
            y_train = y_train[0:129395]
        # import pdb
        # pdb.set_trace()
        ...
        self.fake_targets_train = self._map_new_class_index(self.fake_targets_train, order)
    ...
    def _get_loader(self, x, y, share_memory=None, shuffle=True, mode="train", batch_size=None, resample=None):
        ...
        return DataLoader(DummyDataset(x,
                          ...
                          sampler=sampler,
                          pin_memory=True)
    def _get_fake_loader(self, x, y, shuffle=True):
...
class DummyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'torch' in self.trsf_type:
            ...
            x_trans = self.trsf(image=x)['image']
        return x_trans, y, self.x[idx]
