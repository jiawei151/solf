...
def finetune_last_layer(
    ...
    params = []
    network.eval()
    for param in network.classifier.params(): #TODO: module
        # print(key)
        params += [{"params": [param]}]

    if emanet is not None:
        emanet.eval()
        for param in emanet.classifier.params():
            # print(key)
            params += [{"params": [param]}]
    ...
    for i in range(nepoch):
        ...
        for inputs, targets, _ in loader:
            ...
            if emanet is not None:
                outputs_ema = emanet(inputs)['logit']
                _, preds = outputs_ema.max(1)
            optim.zero_grad()
            loss = criterion(outputs / temperature, targets)
            if emanet is not None:
                loss_ema = criterion(outputs_ema / temperature, targets)
                (loss+loss_ema).backward()
            else:
                loss.backward()
            optim.step()
            if emanet is not None:
                total_loss += loss_ema * inputs.size(0)
            else:
...
def finetune_last_layer_imgnet(
    logger,
    network,
    loader,
    n_class,
    ...
    params = []
    network.eval()
    for param in network.classifier.parameters(): #TODO: module
        # print(key)
        params += [{"params": [param]}]

    if emanet is not None:
        emanet.eval()
        for param in emanet.classifier.parameters():
            # print(key)
            params += [{"params": [param]}]
