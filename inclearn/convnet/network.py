...
class BasicNet(MetaModule):
    def __init__(
        ...
        self.weight_normalization = cfg['weight_normalization'] #false
        ...
        self.der = cfg['der']
        self.aux_nplus1 = cfg['aux_n+1'] #true
        self.reuse_oldfc = cfg['reuse_oldfc'] #false
        ...
        if cfg['postprocessor']['enable']: #False
    ...
    def del_classes(self, n_classes):
        self.ntask -= 1
    ...
    def add_classes_ema(self, n_classes):
        self.ntask += 1
    ...
    def _gen_classifier(self, in_features, n_classes):
        if self.weight_normalization:
            classifier = MetaCosLinear(in_features, n_classes, bias=False).to(self.device)
            print('use cos classfier')
        else:
            classifier = MetaLinear(in_features, n_classes, bias=self.use_bias).to(self.device)
...
class BasicNet_imgnet(nn.Module):
    def __init__(
        self,
        convnet_type,
        cfg,
        ...
        super(BasicNet_imgnet, self).__init__()
        self.nf = nf
        self.init = init
        self.convnet_type = convnet_type
        self.dataset = dataset
