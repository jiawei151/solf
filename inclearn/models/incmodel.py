...
class IncModel(IncrementalLearner):
    ...
    def _get_ttrans(self):
        Ws = []
        for i in range(self._n_classes-self._task_size):
            X = self._data_memory[self._targets_memory == i]
            # print(i, X.shape)
            # if X.shape[0] > 512:
            #     X = X[:512]
            self.eval()
            with torch.no_grad():
                X_o = torch.cat((self.ema_network(X)['feature'].detach(),torch.ones((X.shape[0],1),dtype=torch.float32, device=self._device)),1)
                X_n = self._network(X)['feature'].detach()
                X_oX_oT = torch.inverse(torch.mm(X_o,X_o.t())+torch.ones(X_o.shape[0],X_o.shape[0]).cuda())
                W = torch.mm(torch.mm(X_o.t(), X_oX_oT),X_n) #todo: need ridge?
                Ws.append(W)
        # testing
        self._Ws = Ws
        sums = 0
        ...
        acc_preds = 0
        for i in range(self._n_classes-self._task_size):
            X = self._fake_memory['x'][self._fake_memory['y'] == i]
            self.eval()
            with torch.no_grad():
                out_o = self.ema_network(X)
                out_n = self._network(X)
                Z_o = out_o['feature'].detach()
                Z_n = out_n['feature'].detach()
                logit_n = out_n['logit']

                Z_o_hat = torch.cat((Z_o,torch.ones((X.shape[0],1),dtype=torch.float32, device=self._device)),1)
                Z_n_pred = torch.mm(Z_o_hat,Ws[i])
                logit_n_pred = torch.mm(Z_n_pred, self._network.classifier.weight.data.t())

                acc_gt = (logit_n.argmax(1) == self._fake_memory['y'][self._fake_memory['y']==i]).sum() / logit_n.shape[0]*100
                acc_pred = (logit_n_pred.argmax(1) == self._fake_memory['y'][self._fake_memory['y']==i]).sum() / logit_n.shape[0]*100

                sums += (Z_n-Z_n_pred).pow(2).mean()
                acc_gts += acc_gt
                acc_preds += acc_pred

        res = [sums/(self._n_classes-self._task_size),acc_gts/(self._n_classes-self._task_size),acc_preds/(self._n_classes-self._task_size)]
        return res 
