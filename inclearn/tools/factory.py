...
from inclearn.convnet import resnet, cifar_resnet, modified_resnet_cifar, preact_resnet, resMeta
...
def get_convnet(convnet_type, **kwargs):
    if convnet_type == "resnet18":
        ...
        return resMeta.resnet18(**kwargs)
...
def get_model(cfg, trial_i, _run, ex, tensorboard, inc_dataset):
    ...
    if cfg["model"] == "generative":
        return models.Generative(cfg, trial_i, _run, ex, tensorboard, inc_dataset)
    ...
    if cfg["model"] == "baseline":
        return models.BaseLine(cfg, trial_i, _run, ex, tensorboard, inc_dataset)
    if cfg["model"] == "model_imagenet":
        return models.model_img(cfg, trial_i, _run, ex, tensorboard, inc_dataset)
    if cfg["model"] == "regularization":
        return models.regularization(cfg, trial_i, _run, ex, tensorboard, inc_dataset)
    if cfg["model"] == "ablation":
        return models.ablation(cfg, trial_i, _run, ex, tensorboard, inc_dataset)
    else:
