...
def compute_accuracy_ema(ypred, ytrue, increments, n_classes):
    all_acc = {"top1": {}, "top5": {}}
    ypred, ypred_ema = ypred
    all_acc_ema = {"top1": {}, "top5": {}}
    topk = 5 if n_classes >= 5 else n_classes
    ncls = np.unique(ytrue).shape[0]
    if topk > ncls:
        topk = ncls
    all_acc_meter = ClassErrorMeter(topk=[1, topk], accuracy=True)
    all_acc_meter_ema = ClassErrorMeter(topk=[1, topk], accuracy=True)
    all_acc_meter.add(ypred, ytrue)
    all_acc_meter_ema.add(ypred_ema, ytrue)
    all_acc["top1"]["total"] = round(all_acc_meter.value()[0], 3)
    all_acc_ema["top1"]["total"] = round(all_acc_meter_ema.value()[0], 3)
    all_acc_ema["top5"]["total"] = round(all_acc_meter_ema.value()[1], 3)
    ...
    for i in range(len(increments)):
        if increments[i] <= 0:
            ...
            cur_acc_meter_ema = ClassErrorMeter(topk=[1, topk_], accuracy=True)
            cur_acc_meter.add(ypred[idxes], ytrue[idxes])
            cur_acc_meter_ema.add(ypred_ema[idxes], ytrue[idxes])
            top1_acc_ema = (ypred_ema[idxes].argmax(1) == ytrue[idxes]).sum() / idxes.shape[0] * 100
            ...
            all_acc_ema["top1"][label] = round(top1_acc_ema, 3)
            all_acc_ema["top5"][label] = round(cur_acc_meter_ema.value()[1], 3)
            # all_acc[label] = round((ypred[idxes] == ytrue[idxes]).sum() / len(idxes), 3)

    return [all_acc,all_acc_ema]
